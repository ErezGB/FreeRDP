...
static int ffmpeg_sample_rate(const AUDIO_FORMAT* format)
{
	switch (format->wFormatTag)
	{
		case WAVE_FORMAT_ALAW:
...
static int ffmpeg_bit_rate(const AUDIO_FORMAT* format)
{
	switch (format->wFormatTag)
	{
		case WAVE_FORMAT_ALAW:
		...
		case WAVE_FORMAT_ADPCM:
		case WAVE_FORMAT_DVI_ADPCM:
			return format->nAvgBytesPerSec * 8;
...
static BOOL ffmpeg_open_context(FREERDP_DSP_CONTEXT* context)
{
	...
	context->context->sample_rate = ffmpeg_sample_rate(format);
	context->context->block_align = format->nBlockAlign;
	context->context->bit_rate = ffmpeg_bit_rate(format);

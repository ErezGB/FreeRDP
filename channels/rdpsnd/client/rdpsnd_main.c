...
static UINT rdpsnd_send_client_audio_formats(rdpsndPlugin* rdpsnd)
{
	...
	UINT32 dwVolume;
	UINT32 dwVolumeLeft;
	UINT32 dwVolumeRight;
	UINT16 wNumberOfFormats;
	dwVolumeLeft = ((50 * 0xFFFF) / 100); /* 50% */
	dwVolumeRight = ((50 * 0xFFFF) / 100); /* 50% */
	dwVolume = (dwVolumeLeft << 16U) | dwVolumeRight;
                                      ...
                                      UINT16 BodySize)
{
	...
	{
		BOOL rc;
		AUDIO_FORMAT* deviceFormat = calloc(1, sizeof(AUDIO_FORMAT));
		...
		*deviceFormat = *format;
		...
		if (format->cbSize > 0)
		{
			deviceFormat->data = malloc(format->cbSize);

			if (!deviceFormat->data)
			{
				free(deviceFormat);
				return CHANNEL_RC_NO_MEMORY;
			}

			memcpy(deviceFormat->data, format->data, format->cbSize);
		}
...
static DWORD rdpsnd_virtual_channel_client_thread(LPVOID arg)
{
	BOOL running = TRUE;
